...
library ExternalFuncs {
    ...
    */
    ...
    function compound (uint principal, uint ratio, uint n) public pure returns (uint) {
        return ABDKMath64x64.mulu (
            ABDKMath64x64.pow ( //pow - original code
                ABDKMath64x64.add (ABDKMath64x64.fromUInt (1), ABDKMath64x64.divu (ratio, 10**4)), //(1+r), where r is allowed to be one hundredth of a percent, ie 5/100/100
                n), //(1+r)^n
            principal); //A_0 * (1+r)^n
    }
    ...
    function isStakeSizeOk (uint256 amount, uint256 decimals) public pure returns (bool) {
        return
    ...
    function getErrorMsg (string memory text, uint value) public pure returns (string memory) {
        string memory _msg = string(abi.encodePacked(text, Strings.toString(value)));
        return _msg;
    }
}
